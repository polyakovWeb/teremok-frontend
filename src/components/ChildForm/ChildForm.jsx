...
function ChildForm({ mode, setTypeUserForm, selectedChild }) {
  ...
  const onSubmit = async (childInfoData) => {
    if (window.confirm("Уверены, что хотите сохранить изменения?")) {
      ...
      mode === "change"
        ? await axios
            .patch(`${API_URL}/changeChildInfo`, changedChild, {
              headers: {
  ...
  const deleteSelectedChild = async () => {
    if (
      ...
      await axios
        .delete(
          `${API_URL}/deleteChild/${selectedChild._id}/${selectedChild.parentId}`,
          {
            headers: {
              Authorization: window.localStorage.getItem("Authorization"),
            },
  ...
  switch (mode) {
    case "change":
      return (
        ...
        <>
          <h1 className="title profile-wrapper__title">
            Изменение профиля ребенка
          </h1>
          <form
            className="user-info change-child"
    ...
    case "add":
      return (
        ...
        <>
          ...
          <form
            className="user-info create-child"
            onSubmit={handleSubmit(onSubmit)}
          >
            {/* Фамилия */}
            <ProfileField
